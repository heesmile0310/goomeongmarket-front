...
function ProductDetailedPage({ converPrice }) {
  ...
  useEffect(() => {
    fetch(`http://${BASE_URL}/products/goods/${params.id}`, {
      method: 'GET',
      headers: {
    ...
    fetch(`http://${BASE_URL}/products/review/${params.id}`, {
      method: 'GET',
      headers: {
        'Content-type': 'application/json',
      },
  ...
  const wishCountHandler = () => {
    wishAddHandler();
    if (!isWishAdd) {
      fetch(`http://${BASE_URL}/like/addlike`, {
        method: 'POST',
        headers: {
          'Content-type': 'application/json',
      ...
      fetch(`http://${BASE_URL}/like/removelike`, {
        method: 'DELETE',
        headers: {
          'Content-type': 'application/json',
          token: localStorage.getItem('token'),
  ...
  const cartCountHandler = () => {
    cartAddHandler();
    if (!isCartAdd) {
      fetch(`http://${BASE_URL}/cart/update`, {
        method: 'POST',
        headers: {
          'Content-type': 'application/json',
          token: localStorage.getItem('token'),
